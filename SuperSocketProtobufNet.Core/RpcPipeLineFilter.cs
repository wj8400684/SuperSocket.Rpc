...
public sealed class RpcPackageDecoder : IPackageDecoder<RpcPackageInfo>
{
    ...
    {
        RpcPackageInfo Create();
    }

    class DefaultPacketFactory<TPacket> : IPacketFactory
        where TPacket : RpcPackageInfo, new()
    {
        public RpcPackageInfo Create()
        {
            return new TPacket();
        }
    }
    ...
    {
        var commands = RpcPackageInfo.GetCommands();
    ...
    RpcPackageInfo IPackageDecoder<RpcPackageInfo>.Decode(ref ReadOnlySequence<byte> buffer, object context)
    {
