...
public abstract class RpcAsyncRespIdentifierCommand<TPacket, TRespPacket> : IAsyncCommand<RpcSession, RpcPackageInfo>
    where TPacket : RpcPackageWithIdentifier
    where TRespPacket : RpcRespPackageWithIdentifier, new()
{
    ValueTask IAsyncCommand<RpcSession, RpcPackageInfo>.ExecuteAsync(RpcSession session, RpcPackageInfo package) => SchedulerAsync(session, package, session.ConnectionToken);

    protected virtual async ValueTask SchedulerAsync(RpcSession session, RpcPackageInfo package, CancellationToken cancellationToken)
    {
