    ...
    where TPackage : class, IMessage<TPackage>
{
    ...
    {
        return new ValueTask<TPackage>(Parser.ParseFrom(packet.Body));
    }
    ...
    where TReplyPackage : class, IMessage<TReplyPackage>
{
    ...
    /// <returns></returns>
    protected virtual ValueTask<ReadOnlySequence<byte>> OnEncodePackageAsync(TReplyPackage respPacket)
    {
        return new ValueTask<ReadOnlySequence<byte>>(new ReadOnlySequence<byte>(respPacket.ToByteArray()));
    }
    ...
    private async ValueTask ExecuteAsync(RpcClient session, RpcPackageInfo package, TPackage content)
    {
        ...
        try
        {
            replyPackage.Body = await OnEncodePackageAsync(replyContent);
        }
        catch (Exception ex)
        {
...
[Command(Key = (byte)CommandKey.OrderAdd)]
...
[Command(Key = (byte)CommandKey.LoginReply)]
